...
def create_signup( email, scope, questions ):
    """ Add signup to the current scope """
...
def update_signup( email, scope, questions ):
    ...
    signup_db = db.UserSignup.objects.get(email__iexact=email, scope=scope)
...
def create_or_update_signup( email, scope, questions ):
    # check if user is already added to the current scope
    if db.UserSignup.objects.filter(email__iexact=email, scope=scope).exists():
...
def delete_signup( email, scope ):
    if db.UserSignup.objects.filter(email__iexact=email, scope=scope, deleted_at__isnull=False).exists():
...
def get_signup( email, scope ):
    if not db.UserSignup.objects.filter(email__iexact=email, scope=scope, deleted_at__isnull=True).exists():
        raise Exception(u'Signup for {0} not found'.format(email))

    signup_db = db.UserSignup.objects.get(email__iexact=email, scope=scope, deleted_at__isnull=True)
    return _signup2json(signup_db)


def get_signups( scope ):
    signups = db.UserSignup.objects.filter(deleted_at__isnull=True, scope=scope)
